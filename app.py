        ...
        if len(my_players) < n_picks:
            ...
            if len(second_team_selections) < n_picks:
                ...
                if (my_trade_len == 0) | (second_trade_len == 0):
                    ...
                    pre_to_post = pd.concat([your_team_pre_trade[1].T,your_team_post_trade[1].T], axis = 0)
                    ...
                    pre_to_post = pd.concat([their_team_pre_trade[1].T,their_team_post_trade[1].T], axis = 0)
                    pre_to_post.index = ['Pre-trade','Post-trade']
                    st.dataframe(pre_to_post)         
with rank_tab:
  ...
  with h_rank_tab:
      ...
      st.caption('Category scores are expectedweekly win rates given approximate punt-adjusted future picks')
