...
def _eligibility_cached(struct_sig, cand_key, team_key):
    ...
    """
    return check_single_player_eligibility(list(cand_key), [list(t) for t in team_key])


def _struct_sig():
    """Hashable snapshot of the current position-slot structure (cheap; read once per call)."""
    return tuple(sorted(get_position_numbers().items()))
...
def _eligible(candidates, positions, team_players):
    ...
    return [p for p in candidates
            ...
            and _eligibility_cached(sig, tuple(positions.loc[p]), team_key)]
