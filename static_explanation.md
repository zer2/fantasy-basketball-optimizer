...
Ideally player rankings should have some mathematical backbone, which requires a metric quantifying player value. The standard metric for category leagues is called 'Z-scoring', and it is often used to make objective rankings of players. However, when I looked for a mathematical argument explaining why Z-scores are a good choice for fantasy basketball rankings, I could not find one.
