...
import {
    ...
    setLeagueDrafterCount, countBoardCellsWithPlayer, countBoardPlayers,
} from './helpers.mjs'
...
test('draft board entry controls', async t => {
    ...
    try {
        ...
        await t.test('board starts empty with Team 1 on the clock', async () => {
            ...
            assert.equal(await countBoardPlayers(page), 0, 'no board cell should contain a player yet')
        })
