...
def rank_candidates_route(session_id: str, req: EvaluateRequest, response: Response):
    ...
    # Auction vs draft is all-or-nothing: an Auction Mode session must get per-team remaining_cash
    ...
    # The session's mode is patched whenever the user switches modes; a cash_per_team value left
    # over from an earlier auction lingers harmlessly — it is only consulted in Auction Mode.
    is_auction_league = session.current_params.get('mode') == 'Auction Mode'
    ...
    if is_auction_league and session.current_params.get('cash_per_team') is None:
        raise HTTPException(
            status_code=400,
            detail='cash_per_team must be set on the session for Auction Mode evaluates.',
        )
