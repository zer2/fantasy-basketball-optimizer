...
![Category format selection](img/formats.png)

The website's implementation of H-scoring supports the three common category formats for fantasy basketball: H2H Each Category, H2H Most Categories, and Rotisserie. It does not have native support for any additional variants, like using a specific category as a tiebreaker. 

It also supports any combination of categories, across the default nine categories and several alternative options.

![Category selection options](img/categories.png)
...
The H-scoring algorithm has three input parameters- $\omega$, $\gamma$, and the number of iterations- which are configurable by the user through the sidebar. Different parameter choices will lead to different H-scoring results.
...
![Punt control parameters](img/puntcontrol.png)
...
??? note "What the algorithm is doing while it runs"
    While the spinner is up, the algorithm is iterating, attempting to repeatedly improve its solution. Mathematically, the underlying iteration process is a procedure called gradient descent. 
    ...
    Gradient descent requires an underlying function which has well-defined gradients. The main machinery of H-scoring is the definition of that function. 
...
Different kinds of categories deal with fantasy points differently, and necessitate different formulations for the overall H-score. In "Each Category", teams rotate opponents weekly and keep all the fantasy points they win. For that format, the H-score is defined as the average expected win rate across categories.
...
![Most Category H-score table](img/hmc.png)
/// caption
...
Winning a league is harder than winning a match-up, so H-scores are systematically lower for Rotisserie than for the head-to-head formats. The average is around 8% instead of 50%. 

??? note "How it optimizes each format"
    ...
    For Each Category, the objective function is just the sum of probabilities of winning each category. It is relatively simple to calculate, and calculate the gradient of. Running the algorithm for Each Categories takes barely more than half a second. 
    ...
    The gradient of the MC objective is the same as for Each Categories except multiplied by a 'tipping point' probability, which is the likelihood that any given category will end up being decisive. It is calculated in much the same way as the overall objective is calculated, with a dynamic programming approach to calculate the tipping point probability for each category. 

    The way this calculation works is clever, but still mathematically intensive. For that reason, the algorithm runs significantly more slowly when the format is Most Categories. It takes more than a full second to complete. 

    The Rotisserie objective is mathematically complicated, though actually not much more difficult to compute and differentiate than the Each Categories objective. 
...
When the format is Rotisserie, category-level H-scores are expected fantasy point totals per category instead of the likelihood to win a single matchup. These are equivalent to the individual matchup likelihoods, multiplied by the number of opponents plus one. 

??? note "Why does it punt some categories?"
...
The main H-score table gives only indirect insight into the strategies that H-scoring wants to use with each candidate player. The H-score details tab explains the strategy for individual players more directly. 
...
The category weightings displayed in the first row are based on H-scoring's internal model of how drafting works. It assumes that the drafter will use those weights exactly for candidates going forward, and it also assumes that those weights will have a certain influence on the aggregate statistics of future picks.

??? note "How does it pick category weights for future picks?"
...
??? note "Why is a punted category's weight still high?"
    ...
    In practice, the assumption that players are taken in G-score order is likely particularly problematic. Other managers may also punt, changing the distribution of available players and necessitating more extreme punting strategies than the algorithm understands. 
...
??? note "How does H-scoring decide its positional strategy?"
    The first step in formulating a positional strategy is deciding how to think about players already drafted. This is done through a side process, rather than being integrated with the overall optimization framework. 
...
The exact position structure that H-scoring assumes is configurable by the user
