...
In the case of fantasy basketball, scenarios where the drafter has a significant disadvantage in terms of playing time are largely irrelevant because they are going to lose no matter what. It may be right to not consider those scenarios, when thinking about how useful turnovers really are. But if it is, then the reverse argument should also apply: if the drafter has a significant advantage in playing time, they are going to win no matter what, and those scenarios should be excluded too. 

The only relevant scenarios to strategize around are those where the outcome is close. Gor the outcome to be close, each drafter must win some share of the counting statistics, making it unclear which team has the advantage in turnovers. So this intuition does not make it obvious how to think about turnovers, at least to me
